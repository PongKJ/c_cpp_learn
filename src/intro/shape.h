...
struct Color {
    ...
    Color( const Color& rhs ) : m_red( rhs.m_red ), m_green( rhs.m_green ), m_blue( rhs.m_blue ) {
...
public:
    Point( double x = 0, double y = 0, const Color& c = Color() ) : Shape( c ), m_x( x ), m_y( y ) {
    ...
    void draw() {
        cout << "draw point with color (" << m_color.m_red << "," << m_color.m_green << "," << m_color.m_blue << ")"
             << endl;
    }
};
...
public:
    Circle( const Point& p, double r, const Color& c = Color() ) : Shape( c ), m_center( p ), m_radius( r ) {
        cout << "constructor of Circle" << endl;
    }
    Circle( const Circle& rhs ) : Shape( rhs ), m_center( rhs.m_center ), m_radius( rhs.m_radius ) {
